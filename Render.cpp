 ...
 * @LastEditTime: 2021-08-19 13:36:14
...
Vector3f castRay(const Vector3f &orig, const Vector3f &dir, const Scene &scene, int depth) {
  ...
  if (auto payload = trace(orig, dir, scene.GetObjects()); payload) {
    ...
    payload->hit_obj->GetSurfaceProperties(hitPoint, dir, payload->index, payload->uv, N, st);
        ...
        hitColor +=
            (payload->hit_obj->energy_attenuation) * calculateBlinnPhongColor(orig, dir, scene, depth, payload) + (1 - payload->hit_obj->energy_attenuation) * hitColor;
  }
